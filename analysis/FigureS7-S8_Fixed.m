%% this code is similar to the code FigureS5-S6
% the only difference is that we directly use the ST (those with a response) participants had
% instead of simulate which ST has a respons or not
clear all;
close all;
clc;        
...
num_start = 50; % 50 different starting positions for optimization
...
Psim = nan(num_sim, num_para); % all simulated value including alpha and beta
...
for t = 1:num_sim
    ...
    % make sure tau is not shorter than the minimum speed estimated when
    % considering all responses are correct without timing constraint
    % to better mimic our data.
    count = 0;
    while count < 1
    ...
    [rsim, rtsim] = simulation_nogo_piecewise_mle_fixedST(st, R, tau, mu_rt, sigma_rt, tau_rt, beta_rt,sigma_ta, beta_ta);
    Psim(t,:) = [tau, mu_rt, sigma_rt, tau_rt, beta_rt,sigma_ta, beta_ta];
   ...
   % RTsim(t,:) = rtsim;
    ...
    for j = 1:num_start
        ...
        if ~isinf(init_negLL)
            ...
            %[X(j,:), fval(j)] = fminsearch(obFunc,X0);
        else
            X(j,:) = ones(1, num_para); fval(j) = 9999;
        end
        %[X(j,:), fval(j)] = fminsearch(LL,para_init,options);
    end
...
Symbols = {'\tau' '\mu_{rt}' '\sigma_{rt}' '\delta_{rt}' '\beta_{rt}' '\sigma_{t}' '\beta_{t}'};
